...
public class ContatoCrudJDBC {
	...
	public void salvar(Contato contato) {
		...
		try {
			...
			insereSt.executeUpdate(); //executeUpdate é usado quando temos umas intrução SQL para uma operação de manipulação
									  //de dados : insert,update ou delete .
	...
	public List<Contato> listar() {
		...
		try {
			consulta = conexao.createStatement(); //createStatement é um utilizado quando queremos executar uma instrução SQL, mas sem passagem de parametros.
			resultado = consulta.executeQuery(sql);//executeQuery retorna um resultado ResultSet,por isso é o metodo utilizado
												   //para consultas no banco de dados, como um Select por exemplo.
	...
	public Contato buscaContato(int valor) { // este metodo vai retornar um contato, passando como parâmetro o código que serve de chave primaria da tabela.
		Connection conexao = this.geraConexao();
	...
	public static void main(String[] args) {
		...
		beltrano.setDataCadastro(new Date(System.currentTimeMillis())); // retorna o horario atual em milisegundos
		...
		//Criando um terceiro contato
		Contato ciclano = new Contato();
		ciclano.setNome("Ciclano da Silva");
		ciclano.setTelefone("(43 3339-8126");
		ciclano.setEmail("flavionfg@gmail.com");
		ciclano.setDataCadastro(new Date(System.currentTimeMillis()));
		ciclano.setObservacao("Novo Contato - Criado apenas para teste, não tem no livro");
		contatoCRUDJDBC.salvar(ciclano);
		
		//Testando o metodo de excluir 
		
//		Contato Testando_metodo_excluir = new Contato();
//		Testando_metodo_excluir = contatoCRUDJDBC.buscaContato(1);
//		contatoCRUDJDBC.excluir(Testando_metodo_excluir);
//		System.out.println("Contato excluido " + "Nome do Contato : " + Testando_metodo_excluir.getNome().toString() + " ID : " + Testando_metodo_excluir.getCodigo().toString());
